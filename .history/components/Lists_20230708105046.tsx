    ...
    <div className=" flex md:px-5">
        ...
        <div className="p-4">
            {results.map((pageResult, i) => (
                <div key={pageResult.job_id} className=" grid gap3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    ...
                    {pageResult?.content?.results?.organic?.map((item) => (
                        <Link href={item.url.includes("url?url#") ? item.url.split("url?url#")?.[1] 
                            :
                                ...
                                className="border rounded-xl flex p-4 gap-2 flex-col hover:shadow-xl transition duration-200 ease-out"
                        >
                            <div>
