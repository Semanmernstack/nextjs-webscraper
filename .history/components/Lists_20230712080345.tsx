    ...
    <div className=" flex md:px-5">
                ...
                <div key={pageResult.job_id} className="space-y-3">
                    {pageResult.content.results.filters?.map((filter, i) => (  
                        <div key={i} className="border-2 rounded-r-xl md:rounded-xl p-4">
                            <p className="font-extrabold">{filter.name}</p>
                            <div className="flex flex-col">
                            {filter.values?.map((value) => (
                                
                                <Link key={value.url}  className="font-bold" href={`https://www.google.com${value.url}`} prefetch={false} >
                                    {value.value} 
                                </Link>
        ...
        <div className="p-4">
            {results.map((pageResult, i) => (
                <div key={pageResult.job_id} className=" grid gap-3 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    ...
                    {pageResult?.content?.results?.organic?.map((item) => (
                        <Link href={item.url.includes("url?url=") ? 
                                ...
                                className={`border rounded-xl flex p-4 gap-2  flex-col hover:bg-zinc-900 hover:text-white hover:shadow-xl transition duration-200 ease-out
